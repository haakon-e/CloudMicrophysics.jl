...
function test_riming_jacobian_conserves_condensate(FT)
    ...
    rs_zero = (; ∂ₜq_lcl_frz = zero(FT), ∂ₜq_lcl_shd = zero(FT), ∂ₜq_rai_frz = zero(FT),
        ∂ₜb_rim_lcl = zero(FT), ∂ₜb_rim_rai = zero(FT), f_wet = zero(FT))
    ...
    @testset "the riming Jacobian block conserves condensate ($FT)" begin
        for s in states
            ...
            # EXACT: the block writes nine entries via `_riming_jacobian_block`, plus BIWET's
            # own self-term directly on `(8, 8)` (card #17 - a relaxation toward a fixed
            # density, not a donor transfer, so it bypasses the block). Ten entries total, no
            # others. Every other accumulator is the same expression in both calls, so anything
            # else moving is a leak.
            written = ((1, 1), (5, 1), (3, 1), (3, 3), (5, 3), (7, 1), (7, 3), (8, 1), (8, 3), (8, 8))
    ...
    @testset "the melt drain is ray-form [FT=$FT]" begin
        ...
        @testset "the production substep" begin
            ...
            for δ in FT[0, 1e-3, 2e-2]            # at, just above, and well above solid ice
                ...
                # the drain's implied density IS the bulk quotient, at every δ including the
                # states where the consumer clamp binds and the replaced form drained at ρ_i
                @test pp.ice_melting.q_rim / pp.ice_melting.b_rim ≈ q_rim / b_rim rtol =
                    8 * eps(FT)
                # equivalently: one fractional loss, both moments
                @test pp.ice_melting.q_rim / q_rim ≈ pp.ice_melting.b_rim / b_rim rtol =
                    8 * eps(FT)
                ...
                # the Jacobian's two rime diagonals are the SAME exact fractional loss, which is
                # what makes the implicit solve rescale the pair instead of reshaping it. Both
                # ray-form drains write them: above freezing the entry suppresses ice DEPOSITION
                # but not sublimation, so the diagonal is the sum of the two fractional losses and
                # asserting the melt term alone would be asserting the wrong quantity.
                J = BMT._jacobian_2mp3_manual(g, x, pp, rs)
                @test J[7, 7] == J[8, 8]
        ...
        @testset "inert on a consistent state" begin
            ...
            for (F_rim, ρ_rim) in ((FT(0.3), FT(400)), (FT(0.9), FT(700)))
                ...
                # the replaced form, evaluated at the same state
                old_q = -(-pp.ice_melting.q_ice) * st.F_rim
                old_b = -(-pp.ice_melting.q_ice) * st.F_rim / st.ρ_rim
                @test pp.ice_melting.q_rim ≈ old_q rtol = 32 * eps(FT)
                @test pp.ice_melting.b_rim ≈ old_b rtol = 32 * eps(FT)
            end
        end
    end
end
...
zeros `f_wet` (the collision integral has nothing to collect) - so none of them exercise
this diagonal. Here cloud AND rain collection are both active, `f_wet > 0`, and `BIWET`'s
own self-term `-f_wet/τ_wet` is exact and closed-form with `state`/`logλ`/the quadrature
...
function test_biwet_diagonal_restored(FT)
    ...
    @testset "BIWET's self-term is on the diagonal [FT=$FT]" begin
        ...
        @test J_m[8, 8] ≈ -rs.f_wet / τ_wet rtol = 8 * eps(FT)

        # (2) the ray-form symmetry every OTHER riming test in this file exercises does
        # NOT hold here - correctly, per the docstring above
        @test J_m[7, 7] != J_m[8, 8]
            ...
            for ΔT in resolved
                ...
                ad = FD.derivative(T) do t
                    ...
                    SVector{8}(Tuple(BMT._ice_melting_species(
                        ρ, zero(FT), zero(FT), m.dNdt, m.dLdt, m.melt_frac)))
                end
    ...
    @testset "the melt fraction is conduction-bounded [FT=$FT]" begin
        @testset "a subnormal-mass state stays finite" begin
            ...
            species = BMT._ice_melting_species(
                ρ, FT(0), FT(0), melt.dNdt, melt.dLdt, melt.melt_frac)
