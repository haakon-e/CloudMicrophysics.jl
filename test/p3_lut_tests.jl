...
function test_p3_lut_self_collection_table(FT)
    @testset "ice self-collection lookup table" begin
        ...
        @test all(isfinite, tab.logI)

        @testset "at a grid node the lookup returns the stored value" begin
            ...
            for (ix, jf, kr, ma) in ((1, 1, 1, 1), (5, 3, 4, 2), (nx, nf, nr, na))
                ...
                @test got ≈ exp(tab.logI[ix, jf, kr, ma]) rtol = sqrt(eps(FT))
            end
        end

        @testset "a stored entry is the integrand at that node's own coordinates" begin
            ...
            for (ix, jf, kr, ma) in ((1, 1, 1, 1), (5, 3, 4, 2), (nx, nf, nr, na))
                ...
                @test exp(tab.logI[ix, jf, kr, ma]) ≈ direct rtol = sqrt(eps(FT))
            end
        end

        @testset "a wrongly-shaped array is refused rather than reinterpreted" begin
            ...
            @test_throws ErrorException P3.IceKernelTable(
                zeros(FT, nx, nf, nr, na + 1), grid)
        end

        @testset "an absent population returns exactly zero, taking no logarithm" begin
            st = P3.state_from_prognostic(params, zero(FT), FT(1e5), zero(FT), zero(FT))
            lg = P3.get_distribution_logλ(st)
            @test P3.ice_self_collection(st, lg, vel, one(FT); quad = tab).dNdt == 0
        end

        @testset "the mode carrier delegates faithfully and isolates the tabulated term" begin
            ...
            @test P3.ice_deposition_timescale(vel, aps, tps, T_w, ρₐ, st, lg; quad = bare) === τ_rule
            # and carrying a table for a DIFFERENT term changes this one not at all
            @test P3.ice_deposition_timescale(vel, aps, tps, T_w, ρₐ, st, lg; quad = with) === τ_rule
            ...
            @test P3.ice_self_collection(st, lg, vel, ρₐ; quad = bare).dNdt === sc_rule
            # a table: the carrier routes to it, exactly
            @test P3.ice_self_collection(st, lg, vel, ρₐ; quad = with).dNdt === sc_tab
        end

        @testset "the quadrature path is untouched by the table's existence" begin
            st = P3.state_from_prognostic(params, FT(1e-3), FT(1e5), FT(5e-4), FT(1e-6))
            lg = P3.get_distribution_logλ(st)
...
function test_p3_lut_outputs(FT)
    ...
    @testset "the mode's other three outputs route by their own carrier field ($FT)" begin
        ...
        @test P3.ice_ventilation_integral(vel, aps, ρₐ, st, lg; quad = bare) === v_rule
        @test P3.ice_ventilation_integral(vel, aps, ρₐ, st, lg;
        ...
        @test v_tab != v_rule || isapprox(v_tab, v_rule; rtol = sqrt(eps(FT)))
        ...
        @test P3.ice_terminal_velocity_number_weighted(vel, ρₐ, st, lg; quad = bare) === n_rule
        @test P3.ice_terminal_velocity_mass_weighted(vel, ρₐ, st, lg; quad = bare) === m_rule
        # a table for the OTHER velocity leaves this one exactly on the rule
        @test P3.ice_terminal_velocity_number_weighted(vel, ρₐ, st, lg;
            quad = P3.P3TabulatedQuadrature(rule; vel_m = t_vm)) === n_rule
        @test P3.ice_terminal_velocity_mass_weighted(vel, ρₐ, st, lg;
        ...
        @test P3.ice_terminal_velocity_number_weighted(vel, ρₐ, st, lg; quad = full) ==
              P3.lookup(t_vn, st.ρq_ice / st.ρn_ice, st.F_rim, st.ρ_rim, ρₐ)
        @test P3.ice_terminal_velocity_mass_weighted(vel, ρₐ, st, lg; quad = full) ==
        ...
        @test P3.ice_terminal_velocity_number_weighted(vel, ρₐ, empty, lge; quad = full) == 0
        @test P3.ice_ventilation_integral(vel, aps, ρₐ, empty, lge; quad = full) == 0
    end
end
