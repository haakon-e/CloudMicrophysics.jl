...
The wet-growth-gated shed components (`QCSHD`, `QRSHD`, `∫𝟙_wet_M_col`) are
locked only on the median (near-zero across most states); their `p95`/`max`
inherit the onset-search limitation above and are not useful regression
signals at the default order.
