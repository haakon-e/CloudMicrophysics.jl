...
# `LinearizedAverage` (donor-based-modified system matrix). The accuracy
...
function test_rosenbrock_1m_mode(FT)
    ...
    # vs a 4096-substep reference, regime-calibrated.
    T_frz = TDI.T_freeze(tps)
    regimes = (
        (; ρ = FT(1.0), T = T_frz + FT(17), q_tot = FT(0.018),
            x = FT[2e-3, 0, 5e-4, 0], tol = 0.005),  # warm rain
        (; ρ = FT(1.2), T = T_frz + FT(5), q_tot = FT(0.012),
            x = FT[5e-4, 2e-4, 3e-4, 3e-4], tol = 0.015),  # mixed warm
        (; ρ = FT(1.2), T = T_frz - FT(10), q_tot = FT(0.012),
            x = FT[3e-4, 5e-4, 2e-4, 4e-4], tol = 0.08),  # mixed cold
        (; ρ = FT(1.2), T = T_frz - FT(15), q_tot = FT(0.008),
            x = FT[0, 0, 0, 1e-4], tol = 0.08),  # snow cold deposition
    )
    ...
    @testset "1M substeps stay finite and non-negative ($FT)" begin
        # out-of-equilibrium all-species state, large Δt
    ...
    @testset "1M near-empty species take the explicit path ($FT)" begin
...
# On the exact-AD 1M path `_microphysics_source_terms` runs on `ForwardDiff.Dual`
# species; every per-process rate must return a single concrete type, not a
# `Union{FT, Dual}`.
@testset "1M exact-AD source terms are Union-free" begin
    for FT in (Float64, Float32)
        tps = TDI.TD.Parameters.ThermodynamicsParameters(FT)
        mp = CMP.Microphysics1MParams(FT)
        mkdual(v, i) = FD.Dual{Nothing}(FT(v), ntuple(j -> FT(i == j), 4)...)
        (q_lcl, q_icl, q_rai, q_sno) = ntuple(i -> mkdual(3e-4, i), 4)
        argt = typeof.((
            BMT.Microphysics1Moment(), mp, tps, FT(1.2), FT(278), mkdual(0.012, 0),
            q_lcl, q_icl, q_rai, q_sno,
        ))
        RT = only(Base.return_types(BMT._microphysics_source_terms, argt))
        @test RT <: NamedTuple
        @test !any(ft -> ft isa Union, fieldtypes(RT))
    end
end
