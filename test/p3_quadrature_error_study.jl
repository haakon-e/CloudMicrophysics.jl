...
quadrature rule stored in `mp.ice`. Return `(out, f_wet)`: `out` maps each
RATE quantity to its component vector as before (ice quantities absent for
ice-free states); `f_wet` (card #17, `nothing` where ice is absent) is
...
`f_wet` is a DIAGNOSTIC FRACTION, not a rate, and its own numerator
(`∫𝟙_wet_M_col`) is already on this study's documented exclusion list above -
...
see `run_quadrature_error_study`'s own `f_wet` block for where it prints.
"""
...
function run_quadrature_error_study(;
    ...
    # `f_wet` (card #17) gets its own floor, on the SAME `1e-9 x max-reference-magnitude` recipe
    ...
    f_wet_floor = FT(1e-9) * max(maximum(abs, filter(!isnothing, f_wet_refs); init = FT(0)), eps(FT))
    ...
    for n in orders
        ...
        f_wet_errs = FT[]
        for (si, (s, r, f_wet_ref)) in enumerate(zip(states, refs, f_wet_refs))
            e, f_wet_e = evaluate_quadrature_levels(mp, tps, s)
            for k in keys(r)
            ...
            if f_wet_ref !== nothing && f_wet_e !== nothing
                push!(f_wet_errs, abs(f_wet_e - f_wet_ref) / max(abs(f_wet_e), abs(f_wet_ref), f_wet_floor))
            end
        end
        ...
        # REPORTED, NOT LOCKED (card #17): f_wet inherits the wet-growth onset-search
        # sensitivity this study's own docstring already documents for `∫𝟙_wet_M_col` and its
        ...
        if !isempty(f_wet_errs)
            println("  f_wet (reported, not locked - onset-search-limited like ∫𝟙_wet_M_col): median=",
                round(median(f_wet_errs), sigdigits = 3), " p95=",
                round(quantile(f_wet_errs, 0.95), sigdigits = 3), " max=",
                round(maximum(f_wet_errs), sigdigits = 3), " n=", length(f_wet_errs))
        end
    end
