...
function test_bulk_microphysics_2m_tendencies(FT)
    ...
    @testset "BulkMicrophysicsTendencies 2M - Condensation and Evaporation" begin
        ...
        # Mass-free (q_lcl = 0) with `n_lcl = 1e8` droplets present, and supersaturated: this IS a
        # droplet population, so it condenses. The assertion has been through both errors. It first
        # read `> 0` and was asserting the 1e10-cap leak, `sat_excess / (1e10 · Γₗ)`, 1.6e-14 kg/kg/s
        # of cloud mass minted onto a cell with no droplets at all; the degeneracy gate then made it
        # `== 0`, correctly for an empty cell but not for this one, because the switch in
        # `log_pdf_cloud_parameters_mass` fired on `q_lcl < eps(FT)` whatever the number was. With
        # presence keyed on the number, the population is visible and the rate is a real relaxation:
        # far above the old leak, and finite.
        tend_cond_populated = BMT.bulk_microphysics_tendencies(
            BMT.Microphysics2Moment(),
            mp, tps, ρ, T, q_tot_super, q_lcl, n_lcl, q_rai, n_rai,
        )
        @test tend_cond_populated.dq_lcl_dt > FT(1e-9)
        @test isfinite(tend_cond_populated.dq_lcl_dt)

        # The genuinely empty cell condenses nothing: with no droplets the capacitance integral
        # underflows and the whole condensation slot is gated. Its liquid mass source is then
        # exactly the droplet activation seed, `x_seed` per activated droplet and nothing else,
        # which is what this asserts. Activation does not need an updraft to fire - only its
        # adiabatic-parcel branch does, and this cell is supersaturated over liquid.
        x_seed = mp.warm_rain.seifert_beheng.pdf_c.xc_min
        tend_cond_empty = BMT.bulk_microphysics_tendencies(
            BMT.Microphysics2Moment(),
            mp, tps, ρ, T, q_tot_super, q_lcl, FT(0), q_rai, n_rai,
        )
        @test tend_cond_empty.dn_lcl_dt > FT(0)
        @test tend_cond_empty.dq_lcl_dt === FT(x_seed) * tend_cond_empty.dn_lcl_dt

        # The original intent - supersaturated cloud grows - asserted at a state that has droplets
        # to grow. Same 5% supersaturation, since q_vap = q_tot − q_lcl − q_rai.
        q_lcl_pop = FT(1e-4)
        tend_cond = BMT.bulk_microphysics_tendencies(
            BMT.Microphysics2Moment(),
            mp, tps, ρ, T, q_tot_super + q_lcl_pop, q_lcl_pop, n_lcl, q_rai, n_rai,
        )
