...
function build_state(::Type{FT}; ρ, T, q_lcl, n_lcl, q_rai, n_rai, q_ice, n_ice, F_rim, ρ_rim) where {FT}
    ...
    q_vs = T > TDI.TD.Parameters.T_freeze(tps) ?
...
from `ρ_i`), near-degenerate populations (trace ice near the `eps(FT)` guard), and
...
function corner_states(::Type{FT}, mp) where {FT}
    ...
    # pushing the onset diameter into the bulk of the ice PSD (`derivation_thread1_thread2.md`).
    ...
    # strongly from ρ_i at the graupel/partially-rimed thresholds (`derivation_thread3_massregime.md`).
    ...
    # Near-degenerate: trace ice just above the `eps(FT)` has-ice guard, at both
    # very low and very high number concentration (extreme mean-size tails).
...
scenarios (same state at index `i` in both), differing only in field type. This
matters for any cross-precision comparison: sampling and filtering independently
per `FT` lets `realizable` reject a different subset in each precision (the
acceptance/rejection history diverges after the first mismatch), silently
misaligning state indices across precisions — confirmed to happen in practice
(2 extra rejections in `Float32`) and invalidating any state-by-state `F64` vs
`F32` pairing built on it.
"""
