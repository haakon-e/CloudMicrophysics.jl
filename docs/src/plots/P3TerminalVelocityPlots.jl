...
function get_values(
    ...
    for i in 1:x_resolution
        for j in 1:y_resolution
            ...
            V_m[i, j] =
                P3.ice_terminal_velocity_mass_weighted(Chen2022, ρ_a, state, logλ; aspect_ratio = P3.NoAspectRatio())
            V_m_ϕ[i, j] = P3.ice_terminal_velocity_mass_weighted(Chen2022, ρ_a, state, logλ; aspect_ratio = P3.Oblate())
