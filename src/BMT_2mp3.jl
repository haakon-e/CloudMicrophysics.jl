...
"""
    _ice_melting_species(ρ, q_rim, b_rim, dNdt, dLdt, melt_frac)
...
loss they share ([`CMP3.ice_melt_fraction`](@ref)): ice becomes rain, and rime mass and rime
volume drain along the ray through the origin with that same fraction.
...
"""
@inline function _ice_melting_species(ρ, q_rim, b_rim, dNdt, dLdt, melt_frac)
    ∂ₜq_ice_melt = dLdt / ρ
    ∂ₜn_ice_melt = dNdt / ρ
    o = zero(∂ₜq_ice_melt)
    ∂ₜq_rim_melt = -q_rim * melt_frac
    ∂ₜb_rim_melt = -b_rim * melt_frac
...
`(; ∂ₜq_lcl_frz, ∂ₜq_lcl_shd, ∂ₜq_rai_frz, ∂ₜb_rim_lcl, ∂ₜb_rim_rai, f_wet)`, in the per-kg-air
units of the breakdown (`f_wet` dimensionless), all zero where the transfer is inactive. `pp`
...
quadrature is already paid for. `f_wet` carries the bulk wet growth fraction, so the manual
...
@inline function p3_2m_process_rates(
    ...
    if CMP3.ice_population_is_present(state)
        ...
        ice_melting =
            _ice_melting_species(ρ, q_rim, b_rim, melt.dNdt, melt.dLdt, melt.melt_frac)
    else
        liquid_ice_collision = Z()
        riming_split = (;
            ∂ₜq_lcl_frz = o, ∂ₜq_lcl_shd = o, ∂ₜq_rai_frz = o,
            ∂ₜb_rim_lcl = o, ∂ₜb_rim_rai = o, f_wet = o,
        )
        ice_aggregation = Z()
        ice_melting = Z()
    end
...
"""
    _rime_pair_b_new(q_new, b̃, ρ_min, ρ_max)

The rime volume to pair with the already-floored rime mass `q_new`. `b̃` is `b_rim`'s own
unfloored target, returned unchanged (bit-identical) whenever the implied density `q_new / b̃`
...
"""
@inline _rime_pair_b_new(q_new, b̃, ρ_min, ρ_max) =
    UT.nearest_admissible_b(q_new, b̃, ρ_min, ρ_max)
...
@inline function _apply_positivity_floor(
    ...
    x_new = max.(x .+ Δx, 0)
    b̃ = x[idx_b] + Δx[idx_b]
    b_new = _rime_pair_b_new(x_new[idx_q], b̃, ρ_min, ρ_max)
