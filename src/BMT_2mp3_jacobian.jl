...
@inline function _jacobian_2mp3_manual(g, x::MicroState2MP3{FT}, pp, rs) where {FT}
    ...
    # `state`, `logλ` and the quadrature rates are held fixed - `f_wet`, `ρq_ice` and `ρ_i` all
    # come from `state`/`rates`, never from `B_rim` itself - so `∂(BIWET)/∂B_rim = −f_wet/τ_wet`
    # is exact, not a donor-recipe approximation, mirroring the ray-form exactness of the
    # sublimation and orphan self-terms above. `rs.f_wet` is exactly zero where the riming block
    ...
    brim_brim -= rs.f_wet / mp.ice.scheme.τ_wet
