...
# rather than faulting - `exp10(0) = 1` at every log-valued entry, a finite and plausible but
...
P3TabulatedQuadrature(rule; selfcol = nothing, vent = nothing, vel_n = nothing, vel_m = nothing) =
    P3TabulatedQuadrature(rule, selfcol, vent, vel_n, vel_m, rule.n)
...
"""
    p3_lut_carrier(scheme, vel, aps, grid; quad, outputs = (:selfcol, :vent, :vel_n, :vel_m))
...
`outputs` states which terms to tabulate; a term left out is not tabulated and its field stays
`nothing`. Device residence is the caller's responsibility, as for a single table: pass tables
...
function p3_lut_carrier(scheme, vel, aps, grid::IceKernelTableGrid; quad,
    outputs = (:selfcol, :vent, :vel_n, :vel_m))
    for o in outputs
        o in (:selfcol, :vent, :vel_n, :vel_m) ||
            error("unknown P3 lookup-table output $o; expected one of :selfcol, :vent, :vel_n, :vel_m")
    end
    P3TabulatedQuadrature(quad;
        selfcol = :selfcol in outputs ? IceKernelTable(scheme, vel, grid; quad) : nothing,
        vent = :vent in outputs ? ice_ventilation_table(scheme, vel, aps, grid; quad) : nothing,
        vel_n = :vel_n in outputs ? ice_velocity_n_table(scheme, vel, grid; quad) : nothing,
        vel_m = :vel_m in outputs ? ice_velocity_m_table(scheme, vel, grid; quad) : nothing)
end
