...
@inline function regime_value(state::P3State, D, small, unrimed, dense_rimed, graupel, partially_rimed)
    ...
    small, unrimed, dense_rimed, graupel, partially_rimed =
        _promote_regime(small, unrimed, dense_rimed, graupel, partially_rimed)
    #! format: off
...
# Promote the five branch values to a common type, so `regime_value` selects
# concretely even when branches mix parameter (plain) and state (`Dual`) element
# types. Scalars promote directly; `(prefactor, exponent)` pairs component-wise.
@inline _promote_regime(vals::Vararg{Number, 5}) = promote(vals...)
@inline function _promote_regime(a::Tuple, b::Tuple, c::Tuple, d::Tuple, e::Tuple)
    prefactors = promote(first(a), first(b), first(c), first(d), first(e))
    exponents = promote(last(a), last(b), last(c), last(d), last(e))
    return map(tuple, prefactors, exponents)
end
...
function ice_mass_coeffs(state::P3State, D)
    ...
    return regime_value(
        state, D,
        (ρ_i * π / 6, FT(3)),       # small spherical ice
        (α_va, β_va),               # large nonspherical unrimed ice
        (α_va, β_va),               # dense nonspherical rimed ice
        (ρ_g * π / 6, FT(3)),       # graupel (rimed)
        (α_va / max(1 - F_rim, UT.ϵ_numerics_P3_B(FT)), β_va), # partially rimed ice
    )
end
