...
function get_distribution_logλ(state, logλ_guess = nothing, logλ_min = 2, logλ_max = 17)
    ...
    return _solve_shape_logλ(shape_problem, FT, logλ_guess, lo, hi, maxiters)
end
...
closure, μ comes from the slope law; `logλ_core` equals `logλ` when liquid is
off. Under [`CMP.PredictedLiquidFraction`](@ref) the state forms treat `logλ`
as the whole-particle slope ([`get_distribution_logλ_whole`](@ref)) and solve
`logλ_core` from the frozen core at the shared μ
([`get_distribution_logλ_core`](@ref)).
"""
...
get_distribution_shape(state::P3State, logλ) = _distribution_shape(state.params.liquid, state, logλ)
get_distribution_shape(state::P3State) = _distribution_shape(state.params.liquid, state)

# Without liquid fraction the single slope is both whole and core.
_distribution_shape(::CMP.NoLiquidFraction, state::P3State, logλ) =
    get_distribution_shape(state.params, logλ)
_distribution_shape(::CMP.NoLiquidFraction, state::P3State) =
...
function _distribution_shape(::CMP.PredictedLiquidFraction, state::P3State, logλ)
    μ = get_μ(state, logλ)
    return P3Shape(; logλ, μ, logλ_core = get_distribution_logλ_core(state, μ))
end
function _distribution_shape(::CMP.PredictedLiquidFraction, state::P3State)
